...
from srcV2.learn_framework import LFramework
import srcV2.rl.graph_search.beam_search as search
import srcV2.utils.ops as ops
from srcV2.utils.ops import int_fill_var_cuda, var_cuda, zeros_var_cuda, todevice
import time
import logging
...
class PolicyGradient(LFramework):
    ...
    def get_path_attn(self, paths):
        ...
        # Todo 根据语义相似性额外计算一个关系之间的注意力系数。
        rel_embedding = self.kg.get_all_relation_embeddings()
        att_relsim = torch.sigmoid(torch.mm(rel_embedding,rel_embedding.t()))
        # Todo 在根据路径语义的相似度为每个关系添加一个路径级别的注意力。



        self.args.original_attn_value = original_attn_value
        self.args.attn_weight = 1.3*mp_attn_value.cuda()+0.6*att_relsim
    ...
    def loss(self, mini_batch):
        ...
        for idx, sub_mini_batch in enumerate(mini_batch):
            if len(sub_mini_batch) == 0:
                continue
            if self.args.argcn:
                self.kg.update_embedding('0',query=sub_mini_batch[0][2])
