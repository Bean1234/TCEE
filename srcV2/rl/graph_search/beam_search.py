...
from srcV2.utils.ops import unique_max, var_cuda, zeros_var_cuda, int_var_cuda, int_fill_var_cuda, var_to_numpy, todevice
...
def beam_search(pn, e_s, q, e_t, kg, num_steps, beam_size, return_path_components=False):
    ...
    def top_k_action(log_action_dist, action_space):
        ...
        (r_space, e_space, c_space, p_space), _ = action_space
        ...
        next_c = ops.batch_lookup(c_space.view(batch_size, -1), action_ind).view(-1)
        next_p = ops.batch_lookup(p_space.view(batch_size, -1), action_ind).view(-1)
        ...
        return (next_r, next_e, next_c, next_p), log_action_prob, action_offset
    ...
    def top_k_answer_unique(log_action_dist, action_space):
        ...
        c_space = c_space.view(batch_size, -1)
        p_space = p_space.view(batch_size, -1)
        log_action_dist = log_action_dist.view(batch_size, -1)
        beam_action_space_size = log_action_dist.size()[1]
        assert (beam_action_space_size % action_space_size == 0)
        k = min(beam_size, beam_action_space_size)
        next_r_list, next_e_list, next_c_list, next_p_list = [], [], [], []
        ...
        for i in range(batch_size):
            ...
            top_c = c_space_b[top_unique_idx]
            top_p = p_space_b[top_unique_idx]
            next_r_list.append(top_r.unsqueeze(0))
            next_e_list.append(top_e.unsqueeze(0))
            next_c_list.append(top_c.unsqueeze(0))
            next_p_list.append(top_p.unsqueeze(0))
        ...
        next_c = ops.pad_and_cat(next_c_list, padding_value=1.0).view(-1)
        next_p = ops.pad_and_cat(next_p_list, padding_value=0).view(-1)
    ...
    def adjust_search_trace(search_trace, action_offset):
        for i, (r, e,c_s, p_s) in enumerate(search_trace):
            new_r = r[action_offset]
            new_e = e[action_offset]
            new_c_s = c_s[action_offset]
            new_p_s = p_s[action_offset]
            search_trace[i] = (new_r, new_e,new_c_s,new_p_s)

    # Initialization
    r_s = int_fill_var_cuda(e_s.size(), kg.dummy_start_r)
    c_s = int_fill_var_cuda(e_s.size(), 1.0).float()
    p_s = int_fill_var_cuda(e_s.size(), 0)
    seen_nodes = int_fill_var_cuda(e_s.size(), kg.dummy_e).unsqueeze(1)
    init_action = (r_s, e_s, c_s, p_s)
    # path encoder
    pn.initialize_path(init_action, kg)
    if kg.args.save_beam_search_paths:
        search_trace = [(r_s, e_s, c_s, p_s)]
