...
class LFramework(nn.Module):
    ...
    def run_train(self, train_data):
        ...
        for epoch_id in range(self.start_epoch, self.num_epochs):
            ...
            if epoch_id > -1 and epoch_id % self.num_peek_epochs == 0:
                self.eval()
                pt = PrettyTable()
                ...
                h_1, h_3, h_5, h_10, mrr = srcV2.eval.hits_and_ranks(using_examples, dev_score, self.kg.all_objects, verbose=False)
                ...
                # if run analysis, evaluate  test data
    ...
    def load_checkpoint(self, input_file):
        ...
        if os.path.isfile(input_file):
            ...
            checkpoint['state_dict'].pop('kg.relation_embeddings_in')
            checkpoint['state_dict'].pop('kg.relation_embeddings_out')
