...
from srcV2.parse_args import args
from srcV2.data_utils import NO_OP_ENTITY_ID, DUMMY_ENTITY_ID
import logging
...
def hits_and_ranks_vs100(examples, scores, all_answers, corrupts, verbose=False):
    """
    Compute ranking based metrics.
    """
    assert (len(examples) == scores.shape[0])
    ...
    for i, example in enumerate(examples):
        ...
        # mask all false negatives
        try:
            scores[i, corrupts[example]] += 1
        except:
            print(example)
            continue
        scores[i, e2_multi] = 0
        # write back the save prediction
        scores[i, e2] = target_score + 1
...
def hits_and_ranks_by_seen_queries(examples, scores, all_answers, seen_queries, verbose=False):
    ...
    seen_hits_at_1, seen_hits_at_3, seen_hits_at_5, seen_hits_at_10, seen_mrr = hits_and_ranks(seen_exps, scores[seen_ids], all_answers, verbose=False)
    unseen_hits_at_1, unseen_hits_at_3, unseen_hits_at_5, unseen_hits_at_10, unseen_mrr = hits_and_ranks(unseen_exps, scores[unseen_ids], all_answers, verbose=False)
    if verbose:
        logging.info('MRR on seen queries: {:.3f}'.format(seen_mrr))
        logging.info('MRR on unseen queries: {:.3f}'.format(unseen_mrr))
    return seen_hits_at_1,seen_mrr,unseen_hits_at_1, unseen_mrr

def hits_and_ranks_by_seen_entites(examples, scores, all_answers, seen_queries, verbose=False):
    seen_exps, unseen_exps = [], []
    seen_ids, unseen_ids = [], []
    for i, example in enumerate(examples):
        e1, e2, r = example
        if e1 in seen_queries or e2 in seen_queries:
            seen_exps.append(example)
            seen_ids.append(i)
        else:
            unseen_exps.append(example)
    ...
    if verbose:
        logging.info('MRR on seen queries: {:.3f}'.format(seen_mrr))
        logging.info('MRR on unseen queries: {:.3f}'.format(unseen_mrr))
    print('unseen ratio:',len(unseen_exps)/len(examples))
    return seen_hits_at_1,seen_mrr,unseen_hits_at_1, unseen_mrr
...
def hits_and_ranks_by_relation_type(examples, scores, all_answers, relation_by_types, verbose=False):
    ...
    if verbose:
        logging.info('MRR on to-M relations: {:.3f}'.format(to_m_mrr))
        logging.info('MRR on to-1 relations: {:.3f}'.format(to_1_mrr))
...
def link_MAP(examples, scores, labels, all_answers, verbose=False):
        ...
        for i in range(len(ranked_examples)):
            ...
            if label == '+':
                ...
                if e2 in answer_set or e2 in dummy_mask:
                    logging.info('False negative found: {}'.format(triple))
                    offset += 1 
        if num_pos > 0:
    ...
    map = np.mean(aps)
    if verbose:
        logging.info('MAP = {:.3f}'.format(map))
    return map
